...
for z = 1:length(freq)
    if ~isnan(idx_abr(z)) && ~isnan(idx_template(z))
        for j = 1:length(abr_level)
            % Template vs ABR
            abr_xcorr = xcorr(avg_template(:,z),abr(:,j));
            % Template vs Noise
            abr_noise = mean(abr(end-50:end,:),2);  % average last 50 samples across all levels to represent noise floor
    ...
    if ~isnan(idx_abr(z)) && ~isnan(idx_template(z))
        for j = 1:length(abr_level)
            if z_score(z,j) < 3
                ...
                % Linear scaling: 1 at z = 3 and 0.1 at z = z_max
                weights(z,j) = (z_max - z_score(z,j))/z_max;
            end
        end
    end
end
z_score_w = z_score.*weights;
figure; plot(abr_level,z_score,'*-','linewidth',3); hold on; plot(abr_level,3*ones(size(abr_level)),'--','linewidth',3)
end
