...
if freq~=0
    file_check = dir(sprintf('*Q%s_%s_ABRpeaks_manual_%dHz*.mat',num2str(animal),ChinCondition,freq));
else
    file_check = dir(sprintf('*Q%s_%s_ABRpeaks_manual_click*.mat',num2str(animal),ChinCondition));
end
...
elseif ~isempty(filename) && isempty(abr_FIG.parm_txt(9).String) %Save new file if prior files exist
    ...
    if freq~=0
        filename2 = sprintf('Q%s_%s_ABRpeaks_manual_%dHz',num2str(animal),ChinCondition,freq);
        prompt_peak_save = sprintf('\nSaving New File...\n\nSubject: Q%s \nStimulus: %.1f kHz\n',animal, freq/1000);
    else
        filename2 = sprintf('Q%s_%s_ABRpeaks_manual_click',num2str(animal),ChinCondition);
        prompt_peak_save = sprintf('\nSaving New File...\n\nSubject: Q%s \nStimulus: Click\n',animal);
    end
