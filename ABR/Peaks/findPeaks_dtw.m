...
%% FIX CODE HERE: REPETITIVE PEAKS/LATENCIES SAVED ABOVE ALREADY!! 
            % Plotting
            peaks_plot = signal(sig_inds(idx_sig_inds));
            ...
            for k = 1:num_waves % number of waves I-V
                ...
                if ~any(isnan(sig_inds(idx))) && ~any(isnan(full_peaks_plot(idx)))
                    %plot(t_signal(frame_sig(sig_inds_raw(idx))),signal(sig_inds_raw(idx))+offset, shapes(k),'Color', colors(k+4,:),'MarkerSize', 12,'LineWidth', 1.5,'HandleVisibility', 'off'); % ABR
                    plot(t_signal(frame_sig(sig_inds(idx))),full_peaks_plot(idx)+offset, shapes(k),'Color', colors(k+4,:),'MarkerFaceColor', colors(k+4,:),'MarkerSize', 12,'LineWidth', 1.5,'HandleVisibility', show_in_legend); % ABR
                end
                legend_string{k} = sprintf('Wave %s', waves_legend(k));
            end
            plot(t_signal(frame_sig),template_plot+offset,'--','LineWidth',3,'color',[0 0 0 0.25],'HandleVisibility','off');
            ...
            text(-0.05*max(t_signal),offset, sprintf('%d dB', levels(level_counter)), 'FontSize', 18, 'HorizontalAlignment', 'left','FontWeight','bold');
        else
            peaks = nan(1,num_peaks);
            latencies = nan(1,num_peaks);
            plot(t_signal,signal,'LineWidth',3,'Color', colors(CondIND,:),'HandleVisibility','off')
        end
